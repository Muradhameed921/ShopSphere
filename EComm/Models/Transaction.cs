...
namespace EComm.Models
{
    class Transaction
    {
        ...
        {
            GenerateId();
            using (var cmd = Database.CreateCommand("INSERT INTO [transactions]([user_id], [cash]) VALUES (@p1, @p2)"))
            {
        ...
        public void GenerateId()
        {
            ...
            using (var cmd = Database.CreateCommand("SELECT COUNT(*) FROM transactions WHERE CAST(date_created AS DATE) = CAST(GETDATE() AS DATE)"))
            {
                Database.GetConnection().Open();
        ...
        public static List<Transaction> GetTransactions(DateTime date)
        {
            ...
            using (var cmd = Database.CreateCommand(Database.GetProvider().QUERY_GET_TRANSACTIONS))
            {
                // Ensure parameter is in yyyyMMdd format
                Database.BindParameters(cmd, date.ToString("yyyyMMdd"));
                Database.GetConnection().Open();
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                        result.Add(ConvertReaderToTransaction(reader));
                }
                Database.GetConnection().Close();
            }
            return result;
