...
namespace EComm.Models
{
    class Product
    {
        ...
        public static List<Product> GetAllDeleted()
        {
            ...
            using (var cmd = Database.CreateCommand("SELECT * FROM [products] WHERE is_deleted=1"))
            {
                Database.GetConnection().Open();
        ...
        public static List<Product> GetAllProducts()
        {
            ...
            using (var cmd = Database.CreateCommand("SELECT * FROM [products] WHERE [is_deleted]=0"))
            {
                Database.GetConnection().Open();
                using (var reader = cmd.ExecuteReader())
                {
